...
func (r *Runner) SeedAuth(ctx context.Context, cfg config.Config) error {
	if strings.TrimSpace(cfg.AdminPassword) == "" {
		return fmt.Errorf("ADMIN_PASSWORD wajib diisi untuk seeder admin")
	}
	if err := r.authService.EnsureUser(ctx, cfg.AdminName, cfg.AdminUsername, cfg.AdminEmail, cfg.AdminPassword, domain.RoleAdmin); err != nil {
		return err
	}
