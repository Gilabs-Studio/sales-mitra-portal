...
func main() {
	...
	clientProjectService := service.NewClientProjectService(repository, notificationService)

	if err := seeder.New(repository, authService).Run(context.Background(), cfg); err != nil {
		log.Fatalf("run seeders: %v", err)
	}

	router := server.NewRouter(cfg, repository, authService, leadService, knowledgeService, serviceCatalogService, clientProjectService)
