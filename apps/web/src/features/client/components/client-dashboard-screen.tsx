...
function getActivityLabel(notification: ClientNotification) {
  if (notification.targetType === "project_progress") return "Progress";
  if (notification.targetType === "project_document") return "Dokumen";
  if (notification.targetType === "invoice") return "Invoice";
  if (
    notification.targetType === "maintenance_log" ||
    notification.targetType === "project_maintenance_log"
  )
    return "Maintenance";
  if (
    notification.targetType === "project_maintenance" ||
    notification.targetType === "maintenance"
  )
    return "Maintenance";
  return "Project";
}

function getActivityTone(notification: ClientNotification) {
  if (notification.targetType === "project_progress") return "text-blue-600";
  if (notification.targetType === "project_document") return "text-sky-600";
  if (notification.targetType === "invoice") return "text-rose-600";
  if (
    notification.targetType === "maintenance_log" ||
    notification.targetType === "project_maintenance_log"
  )
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center rounded border px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider ${map[status] ?? "bg-muted text-muted-foreground border-border"}`}
    >
...
export function ClientDashboardScreen() {
  ...
  const handleProjectSelect = (projId: string) => {
    ...
    if (data?.maintenance) {
      const filtered = data.maintenance.filter(
        (m) => m.projectId === projId && m.quotaUsed < m.quotaLimit,
      );
  ...
  const openReportModal = () => {
    ...
    if (data?.projects && data.projects.length > 0) {
      ...
      if (data.maintenance) {
        const filtered = data.maintenance.filter(
          (m) => m.projectId === firstProjId && m.quotaUsed < m.quotaLimit,
        );
        setSelectedMaintId(filtered.length > 0 ? filtered[0].id : "");
      }
    }
    ...
    reportMutation.mutate(
      {
        ...
        payload: { maintenanceId: selectedMaintId, description: reportDesc },
      },
      {
        onSuccess: () => {
  ...
  const availableMaintenance =
    data?.maintenance?.filter(
      (m) => m.projectId === selectedProjectId && m.quotaUsed < m.quotaLimit,
    ) ?? [];
  const combinedActivityItems = notificationItems;
                ...
                <div className="flex flex-col gap-3">
                  {data?.maintenance && data.maintenance.length > 0 ? (
                    data.maintenance.map((m) => {
                      ...
                      const pct =
                        m.quotaLimit > 0
                          ? Math.min((m.quotaUsed / m.quotaLimit) * 100, 100)
                          : 0;
                    ...
                    <div className="rounded-lg border border-dashed border-border p-8 text-center">
                      <p className="text-sm text-muted-foreground">
                        Belum ada paket maintenance aktif.
                      </p>
                    </div>
                  )}
                </div>
              ...
              <div className="flex flex-col gap-4">
                  ...
                  {notificationsQuery.data &&
                    notificationsQuery.data.totalPages > 1 && (
                      <div className="flex gap-1">
                        ...
                        <button
                          disabled={
                            notifPage >= notificationsQuery.data.totalPages
                          }
                ...
                <div className="flex flex-col gap-2">
                  {notificationsQuery.isLoading ? (
                    ...
                    <>
                      {combinedActivityItems.map((notification) => {
                        return (
                          <div
                            key={notification.id}
                          ...
                          >
                            <div className="flex items-center justify-between gap-2">
                              <span
                                className={`truncate text-[10px] font-extrabold uppercase tracking-wider ${getActivityTone(notification)}`}
                              >
                                {getActivityLabel(notification)}
                              </span>
                    ...
                    {availableMaintenance.length > 0 ? (
                      availableMaintenance.map((m) => (
                        <option key={m.id} value={m.id}>
                          {m.packageName} (Sisa: {m.quotaLimit - m.quotaUsed}{" "}
                          req)
                        </option>
                      ))
                    ) : (
                      <option value="">
                        -- Tidak ada paket dengan sisa kuota --
                      </option>
                    )}
                  </select>
                </div>
